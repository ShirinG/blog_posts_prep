...
```{r eval=FALSE, echo=FALSE, include=FALSE}
friends2_df_final_2 <- friends2_df_final
load("friends_friends.RData")
friends2_df_final <- rbind(friends2_df_final, friends2_df_final_2)
...
```{r warning=FALSE, fig.width=10, fig.height=3, echo=FALSE, eval=FALSE}
followers_df %>%
  ...
  select(statusesCount_pDay, followersCount) %>%
  gather(x, y) %>%
  ggplot(aes(x = y)) +
    ...
    labs(x = expression(log[2]~of~{2}^nd~"degree"~"followers, friends & posts per day"),
         y = "density")
```
...
followers_df %>%
  ...
  select(name, followersCount, description, followersCount) %>%
  .[1:10,]
```
...
```{r fig.width=5, fig.height=4, echo=FALSE, eval=FALSE, message=FALSE}
tidy_descr %>%
  count(word_stem, sort = TRUE) %>%
  top_n(10) %>%
  ggplot(aes(x = reorder(word_stem, n), y = n)) +
    geom_col(color = "#377F97", fill = "#377F97", alpha = 0.8) +
    coord_flip() +
    theme_bw() +
    labs(x = "",
...
```{r fig.width=5, fig.height=2.5, echo=FALSE, eval=FALSE}
bigram_counts %>%
  ggplot(aes(x = reorder(word1, -n), y = reorder(word2, -n), fill = n)) +
    geom_tile(alpha = 0.8, color = "white") +
    scale_fill_gradientn(colours = c("#5BB18E", "#377F97")) +
    coord_flip() +
    theme_bw() +
...
not_words <- bigrams_separated %>%
  ...
  count(word2, score.y, sort = TRUE) %>%
  ungroup()
```

```{r fig.width=5, fig.height=2, echo=FALSE, eval=FALSE}
not_words %>%
  mutate(contribution = n * score.y) %>%
  arrange(desc(abs(contribution))) %>%
  mutate(word2 = reorder(word2, contribution)) %>%
  ggplot(aes(word2, n * score.y, fill = n * score.y > 0)) +
    geom_col(show.legend = FALSE) +
    scale_fill_manual(values = c("#5BB18E", "#377F97")) +
    labs(x = "",
...
```{r fig.width=6, fig.height=2.5, echo=FALSE, eval=FALSE}
...
node_diameter <- get.diameter(graph_diameter,  directed = TRUE)

V(graph_diameter)$color <- scales::alpha(V(graph_diameter)$color, alpha = 0.2)
V(graph_diameter)$size <- 1

V(graph_diameter)[node_diameter]$color <- "red"
V(graph_diameter)[node_diameter]$size <- 5

E(graph_diameter)$color <- "grey"
E(graph_diameter)$width <- 1

E(graph_diameter, path = node_diameter)$color <- "red"
E(graph_diameter, path = node_diameter)$width <- 5

V(graph_diameter)$label <- ifelse(V(graph_diameter)$color == "red", V(graph_diameter)$name, NA)
```

```{r fig.width=40, fig.height=40, echo=FALSE, eval=FALSE}
pdf("twitter_net_diameter.pdf", width = 70, height = 80)
plot(graph_diameter,
     layout = layout,
     vertex.label = V(graph_diameter)$label,
     vertex.color = V(graph_diameter)$color, 
     vertex.size = V(graph_diameter)$size, 
     vertex.frame.color = "gray", 
     vertex.label.color = "black", 
     vertex.label.cex = 2,
     edge.arrow.size = 1,
     edge.color = E(graph_diameter)$color,
     edge.width = E(graph_diameter)$width)
dev.off()
```
...
```{r fig.width=12, fig.height=5, echo=FALSE, eval=FALSE}
...
```{r fig.width=12, fig.height=3, echo=FALSE, eval=FALSE}
topics_beta %>%
  ...
  ggplot(aes(term, beta, color = factor(topic))) +
    geom_col(show.legend = FALSE, alpha = 0.8, color = "#377F97", fill = "#377F97") +
...
```{r fig.width=14, fig.height=7, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
save(node_table, file = "node_table.RData")
```

```{r echo=FALSE, eval=FALSE}
load("node_table.RData")
